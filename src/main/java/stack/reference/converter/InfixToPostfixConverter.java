package stack.reference.converter;
	...
	public String convertInfixToPostfix(String infixExpression) {
		...
		for (int index = 0; index < infixExpression.length(); index++) {
			...
			if (Character.isLetter(symbol)) {
				...
				while (!operators.isUnderflow() // stack is not empty
						&& !(operators.peek() == '(') // the ( symbol is not encountered
						&& getPrecedence(symbol) <= getPrecedence(operators.peek()) // the precedence of the current symbol is low
				) {
					postfixExpression.append(operators.pop());
				}
				operators.push(symbol);
